...
def test_fast_scan_can_filter_orders_without_a_printerval_designer(db_session):
    adapter = FakePrintervalAdapter()
    _seed_waiting_order(adapter, "DJ0001001", designer=None)
    _seed_waiting_order(adapter, "DJ0001002", designer="Designer A")
    platform = Platform(name="Test platform", account_username="test@example.com")
    db_session.add(platform)
    db_session.commit()
