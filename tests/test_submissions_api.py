...
def _login(client, db_session, role, username):
    ...
    if not u:
        u = User(username=username, role=role, full_name=f"Test {username}", password_hash=hash_password("s3cret!"))
        db_session.add(u)
        db_session.commit()
    res = client.post("/api/login", json={"username": username, "password": "s3cret!"})
