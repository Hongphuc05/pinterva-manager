...
const KEY = 'dupcompare.search.history.v1'
...
export function useSearchHistory() {
  ...
  const search = useCallback(async (file: File, opts: { refresh?: boolean; replaceId?: string } = {}) => {
    setLoading(true)
    setError(null)
    const url = URL.createObjectURL(file)
    setPending({ name: file.name, url })
    try {
      const [result, thumb] = await Promise.all([searchByImage(file, opts.refresh), makeThumb(file)])
  ...
  /** Search the same file again with a freshly loaded pool (only while the file is still in memory). */
  const retry = useCallback(
    (id: string) => {
      const file = files.current.get(id)
      return file ? search(file, { refresh: true, replaceId: id }) : undefined
    },
    [search],
  )

  const active = entries.find((e) => e.id === activeId) ?? null
  return { entries, active, activeId, setActiveId, loading, error, pending, search, remove, clear, retry, canRetry: (id: string) => files.current.has(id) }
}
