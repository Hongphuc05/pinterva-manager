  ...
  return (
    <div className="flex min-w-0 flex-1 flex-col gap-2">
      <div className="flex items-baseline gap-2">
        ...
        <span className="font-mono text-[13px] font-medium text-brand">{code}</span>
        {name && <span className="min-w-0 flex-1 truncate text-dim" title={name}>{name}</span>}
      </div>
      <div className="relative grid aspect-square w-full place-items-center overflow-hidden rounded-md border border-line bg-white">
        {/^https?:\/\//i.test(src) || src.startsWith('/review/') ? (
          <img src={src} alt={code} className="size-full object-contain" />
        ) : (
          <span className="text-dim">Không có ảnh</span>
        )}
        {overlay}
      </div>
      {children}
...
function ConfigList({ entries }: { entries: { key: string; value: string }[] }) {
  if (!entries.length) return <span className="text-[12px] text-dim">Không có cấu hình</span>
  return (
    <ul className="space-y-0.5 text-[12px] text-dim">
...
export function DetailModal({ item, index, busy, onIndex, onClose, onSelect }: Props) {
  ...
  return (
    ...
    >
      <div
        className="flex max-h-full w-full max-w-5xl flex-col gap-3 overflow-y-auto rounded-lg border border-line bg-surface p-4"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center gap-3">
          <b className="text-sm">Chi tiết đơn</b>
          <span className="font-mono text-[13px] text-brand">{item.order_code}</span>
          ...
          <button
            ...
            aria-label="Đóng"
            onClick={onClose}
            className="grid size-7 cursor-pointer place-items-center rounded border border-line bg-raised text-dim transition hover:text-fg"
          >
            <X className="size-4" />
          </button>
        </div>
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-start">
          <Pane title="Ảnh gốc" code={item.order_code} name={item.product_name} src={proxied(item.image_url)}>
          ...
          <Pane
            ...
            src={proxied(candidate.image_url)}
            overlay={
              <>
                <button
                  type="button"
                  ...
                  className="absolute left-2 top-1/2 grid size-9 -translate-y-1/2 cursor-pointer place-items-center rounded-full border border-line bg-bg/80 text-fg backdrop-blur transition hover:text-brand disabled:cursor-not-allowed disabled:opacity-30"
                >
                ...
                <button
                  ...
                  onClick={() => onIndex(index + 1)}
                  className="absolute right-2 top-1/2 grid size-9 -translate-y-1/2 cursor-pointer place-items-center rounded-full border border-line bg-bg/80 text-fg backdrop-blur transition hover:text-brand disabled:cursor-not-allowed disabled:opacity-30"
                >
          ...
          >
            <div className="flex flex-wrap items-center gap-x-4 gap-y-1 font-mono text-[12px] text-dim">
              <span>
                Độ giống <b className="text-[14px] text-fg">{candidate.similarity.toFixed(3)}</b>
              </span>
            ...
            {canAct && !candidate.sent_to_telegram && (
              <button
                ...
                className="inline-flex h-7 w-fit cursor-pointer items-center rounded bg-brand px-3 text-xs font-medium text-white transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-50"
              >
          ...
          {item.candidates.map((c, i) => (
            <button
              ...
              className={`relative size-14 flex-none cursor-pointer overflow-hidden rounded border bg-white p-0 ${
                i === index ? 'border-brand ring-2 ring-brand/40' : 'border-line opacity-70 hover:opacity-100'
              }`}
            >
              <img src={proxied(c.image_url)} alt="" loading="lazy" className="size-full object-contain" />
