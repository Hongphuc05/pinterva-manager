...
// PrintervalApiAdapter.discover_orders). Status/Designer/Date aren't wired yet: crawling
// is specifically for claiming *new* orders, which only ever matters for "Waiting"
// (an order with a designer/date already set isn't a candidate to claim), so replicating
// those two controls here wouldn't do anything real yet — shown disabled for visual
// parity with the site, not silently ignored.
const JOB_TYPES = ['Tất cả 2D & 3D', '2D', '3D', 'ART', 'WOOD', 'CALENDAR', 'EMBROIDERY', 'AI']

type CrawlFilterModalProps = {
  isOpen: boolean
  onClose: () => void
  onSearch: (jobType: string) => void
  loading: boolean
}
      ...
      >
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="space-y-1 opacity-50">
            ...
            <div className="grid grid-cols-2 gap-3">
              <input disabled placeholder="Date from" className="w-full px-3 py-2 text-xs rounded-xl border border-slate-200 bg-slate-50" />
              <input disabled placeholder="Date to" className="w-full px-3 py-2 text-xs rounded-xl border border-slate-200 bg-slate-50" />
            </div>
            <p className="text-[10px] text-slate-400">Chưa hỗ trợ — cần xác nhận field ngày thật trên Printerval trước khi tự động hoá (xem claude.md §17 #8).</p>
          </div>

          <div className="pt-2 flex justify-end gap-2 border-t border-slate-100">
            <button
              type="button"
              onClick={() => setJobType(JOB_TYPES[0])}
