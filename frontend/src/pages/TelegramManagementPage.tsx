...
type TelegramTemplate = {
  template_key: string
  audience: 'designer' | 'admin' | 'support'
  body: string
  active: boolean
  version: number
...
const templateLabels: Record<string, string> = {
  ...
  support_duplicate_new_candidate: 'Support — Ảnh mới cần kiểm tra trùng',
  support_duplicate_match_candidate: 'Support — Ảnh đối chiếu',
...
export function TelegramManagementPage() {
  ...
  const audienceLabel = selectedTemplate?.audience === 'support'
    ? 'Gửi Support'
    : selectedTemplate?.audience === 'designer'
      ? 'Gửi Designer'
      : 'Gửi Admin'
  const audienceClass = selectedTemplate?.audience === 'support'
        ...
        {loading && !overview ? (
          ...
          <>
            <div className="grid grid-cols-2 gap-3 lg:grid-cols-6">
              ...
              <StatusCard icon={Users} label="Tổng người nhận" value={String(overview.recipient_count)} />
              <StatusCard icon={Users} label="Designer" value={String(overview.designer_count)} />
              <StatusCard icon={Users} label="Support" value={String(overview.support_count)} tone="amber" />
            ...
            <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xs">
              <div className="flex flex-col gap-3 border-b border-slate-100 p-5 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <h2 className="flex items-center gap-2 text-sm font-bold text-slate-800"><Settings2 className="h-4 w-4 text-[#0052CC]" /> Đích gửi theo người nhận</h2>
                  <p className="mt-1 text-xs text-slate-500">Designer có thể dùng group đã xác thực; Support nhận candidate qua chat riêng Telegram.</p>
                </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full min-w-[1120px] text-left text-xs">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {visibleDesigners.map((designer) => {
                      ...
                      const selectedChatId = designer.role === 'support' ? designer.private_chat_id : designer.selected_chat_id
                      return (
                        <tr key={designer.id} className="align-top hover:bg-slate-50/60">
                          ...
                          <td className="px-4 py-4">
                            {designer.role === 'support' ? (
                              <div className="space-y-1">
                                <span className="inline-flex items-center rounded-lg bg-amber-50 px-2 py-1 text-[10px] font-bold text-amber-700">Chỉ chat riêng</span>
                                <p className="max-w-[220px] text-[10px] text-slate-500">Candidate trùng được xác thực theo chat riêng của Support.</p>
                              </div>
                            ) : (
                              <>
                          ...
                          <td className="px-4 py-4">
                            <div className="flex items-center gap-2">
                              <select value={designer.role === 'support' ? 'private' : designer.delivery_mode} onChange={(event) => void changeMode(designer, event.target.value as DeliveryMode)} disabled={Boolean(busy)} className="rounded-lg border border-slate-200 bg-white px-2 py-2 text-[11px] font-bold text-slate-700 outline-none focus:border-[#0052CC] disabled:opacity-50">
                                <option value="private">Chat riêng</option>
                                {designer.role !== 'support' && <option value="group" disabled={!designer.group_verified}>Group chat</option>}
                              </select>
                            </div>
                            <p className="mt-1 max-w-[220px] break-all font-mono text-[10px] text-slate-500">{selectedChatId || 'Chưa có đích gửi'}</p>
                          </td>
                          <td className="px-4 py-4">
                            <button type="button" onClick={() => void sendTest(designer)} disabled={Boolean(busy) || !selectedChatId} className="inline-flex items-center gap-1.5 rounded-lg border border-sky-200 bg-sky-50 px-2.5 py-2 text-[11px] font-bold text-sky-700 hover:bg-sky-100 disabled:cursor-not-allowed disabled:opacity-50">
                ...
                {visibleDesigners.length === 0 && <div className="py-12 text-center text-xs text-slate-400">Không tìm thấy người nhận phù hợp.</div>}
              </div>
            </section>
                ...
                <div className="grid gap-5 pt-5 lg:grid-cols-2">
                  <div className="space-y-3">
                    <div className="flex flex-wrap items-center gap-2 text-[11px] font-semibold text-slate-500">
                      <span className={`rounded-lg px-2 py-1 font-bold ${audienceClass}`}>{audienceLabel}</span>
