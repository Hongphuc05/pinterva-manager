...
export function DuplicateBoardPage() {
  ...
  function autoScrollWhileDragging(event: DragEvent<HTMLElement>) {
    ...
    const column = eventTarget.closest<HTMLElement>('[data-board-column]')
    const cardsContainer = column?.querySelector<HTMLElement>('[data-board-cards]')
    if (!cardsContainer) return

    const bounds = cardsContainer.getBoundingClientRect()
    if (event.clientY < bounds.top + edge) {
      cardsContainer.scrollBy({ top: -maxStep, behavior: 'auto' })
            ...
            {orderedColumns.map((column) => {
              ...
              return (
                <section
                  ...
                  className={`flex h-full min-h-0 w-80 shrink-0 flex-col rounded-xl border p-3 transition-all ${
                ...
                >
                  ...
                  <div
                    data-board-cards
                    aria-label={`Danh sách đơn của ${column.title}`}
                    className="min-h-28 flex-1 space-y-2 overflow-y-auto overscroll-contain pr-1"
                  >
